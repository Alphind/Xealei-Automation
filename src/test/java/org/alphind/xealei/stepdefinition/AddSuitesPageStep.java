...
public class AddSuitesPageStep extends BaseClass {
	...
	public void user_should_clicks_on_the_add_suite_button_and_verifies_that_the_add_suite_popup_screen_is_displayed()
			throws Exception {
	...
	public void user_should_verify_the_limit_error_info_message_for_suite_name_field(
			String expLimitValidationMsgForSN) {
		...
		try {
			...
			Assert.assertEquals("limit error info message for Suite Name* field is NOT displayed",
					expLimitValidationMsgForSN, getText(pom.getAddSuitesPage().getSuiteNameLimit()));
	...
	public void user_should_perform_only_non_mandatory_fields_and_verify_the_info_message_contains_for_mandatory_fields(
			String expSNMandatoryTxt) {
	...
	public void user_should_verify_the_icon_in_add_suites_screen_is_working() throws Exception {
		...
		try {
			Assert.assertEquals("Close popup content is NOT displayed as expected", expContent,
					pom.getAddSuitesPage().getclosePopupContentText());
	...
	public void user_should_perform_login_to_check_suite_limit_with_valid_email_and_password() {

		pom.getLoginPage().validEmail(7);
		pom.getLoginPage().validPassword(7);
		pom.getLoginPage().isEmailAndPasswordFieldIsEmpty();
		pom.getLoginPage().loginButton();
	}

	@Then("User should perform suite by entering only Mandatory field to check suite limit")
	public void user_should_perform_suite_by_entering_only_mandatory_field_to_check_suite_limit() throws Exception {

		logWithLabelName(getMethodName());

		pom.getAddSuitesPage().enterSuiteName(1);
		waitForFullPageElementLoad();

		String expLimitText = "Note: The maximum number of suites has been reached";

		try {
			Assert.assertEquals("NOTE info is NOT display as expected", expLimitText,
					getText(pom.getAddSuitesPage().getmaxSuiteLimitValMsgText()));
			log(Status.PASS, "NOTE info is NOT display as expected -"
					+ getText(pom.getAddSuitesPage().getmaxSuiteLimitValMsgText()));
	...
	public void user_should_verify_all_fields_are_created_successsfully() throws Exception {
		...
		try {
			...
			log(Status.PASS,
					"Suite availability status is displayed as expected Exp Suite Availability status :"
							+ expAvailability + " Act Suite Availability status:"
							+ getText(pom.getAddSuitesPage().getSuiteAvailability()));
	...
	public void user_should_verify_the_breadcrums_link_should_be_display_with_module_suite_name_selected_suite_name()
		...
		if (txtBreadCrum.contains("Suites") && pom.getAddSuitesPage().breadCrumSuiteNameText()
			...
			log(Status.PASS, "Breadcrums link name is displayed as expected :"
					+ pom.getAddSuitesPage().breadCrumSuiteNameText());
